...
std::map<std::pair<float, float>, std::vector<int>> partition(std::vector<City*> cities, std::pair<float, float> *centers, int k) {
    std::map<std::pair<float, float>, std::vector<int>> c;
    //std::cout << "Partitioning start" << std::endl;
    for(int i = 0; i < cities.size(); ++i) {
        std::pair<float, float> best_c = centers[0];
        ...
        for(int z = 1; z < k; ++z) {
            ...
            //std::cout << "tmp_dist calcolata" << std::endl;
        ...
        c[best_c].push_back(i); // non elimina vecchi cluster!
    }
    return c;
}
...
void kmeans(std::vector<City*> &cities, int k, int q) { //Cluster
